...
public class ActivityDaoImpl implements IActivityDao {
    ...
    public boolean chooseOption(Option option) {
        ...
        Object preAnswer = redisTemplate.opsForHash().get(getQuestionOptionKey(questionID), userName);
        redisTemplate.opsForHash().put(getQuestionOptionKey(questionID), userName, userOption.toString());
        boolean isRightAnswer = userOption.equals(rightOption);
        if(preAnswer != null && preAnswer instanceof String) {
            Integer pre = Integer.valueOf(preAnswer.toString());
            if(!pre.equals(userOption)) {
                if (pre.equals(rightOption)) {
    ...
    public Integer getUserOption(Option option) {
        Integer questionID = option.getQuestionID();
        String userName = option.getUserName();
        LOGGER.info("user: {} question {} optionKey {}", option.getUserName(), option.getQuestionID(), getQuestionOptionKey(questionID));
        Object userOptionObject = redisTemplate.opsForHash().get(getQuestionOptionKey(questionID), userName);
        LOGGER.info("userObject {}", userOptionObject);
