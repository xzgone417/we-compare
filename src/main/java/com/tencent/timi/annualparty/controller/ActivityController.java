...
import com.tencent.timi.annualparty.controller.response.TimeResponse;
...
public class ActivityController {
    ...
    public ResponseEntity getQuestionLastTime(@RequestHeader Map<String, String> headerJsonMap,
        ...
        if (expireTime >= 0) {
            return new ResponseEntity(new TimeResponse(0, "答题已开始", expireTime), HttpStatus.OK);
        } else if (expireTime == -1) {
            return new ResponseEntity(new TimeResponse(1, "答题尚未开始", -1), HttpStatus.OK);
        } else {
            return new ResponseEntity(new TimeResponse(2, "答题已经结束", -2), HttpStatus.OK);
        }
    }
